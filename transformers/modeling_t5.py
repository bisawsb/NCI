...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        print("[INFO] Using customized T5.")
        ...
        adaptor_decode = getattr(config, "adaptor_decode", None)
        if decode_embedding:
        ...
        if decode_embedding and hierarchic_decode:
            ...
            self.decoder = self.decoder = HierarchicT5Stack(decoder_config, self.decode_embeddings,
                                                            depth=decode_depth)
        else:
    ...
    def forward(
        ...
        var_list = [decoder_input_ids, decoder_attention_mask, decoder_inputs_embeds,
                    past_key_values, hidden_states, attention_mask,
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=var_list[4],
        ...
        if self.adaptor:
            ...
            adaptor_weight = self.adaptor_linear(adaptor_output).reshape(adaptor_output.shape[0], adaptor_output.shape[1], self.model_dim, self.model_dim)
            ## TODO: add activation?
            lm_head = torch.matmul(adaptor_weight, self.lm_head.weight.T)
            lm_logits = torch.matmul(sequence_output.unsqueeze(-2), lm_head)
        ...
        if labels is not None:
            ...
            if self.Rdrop > 0:
                ...
                orig_loss = loss_fct(lm_logits_half.view(-1, lm_logits.size(-1)), label_half.view(-1))
                p_loss = F.kl_div(F.log_softmax(lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1),
                                  F.softmax(lm_logits_another.view(-1, lm_logits.size(-1)), dim=-1),
                                  reduction='none')
                q_loss = F.kl_div(F.log_softmax(lm_logits_another.view(-1, lm_logits.size(-1)), dim=-1),
                                  F.softmax(lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1),
                                  reduction='none')
                p_loss = p_loss.sum()
                q_loss = q_loss.sum()
                dist_loss = (p_loss + q_loss) / 2
        ...
        return_result = Seq2SeqLMOutput(
            loss=loss,
            logits=lm_logits,
            past_key_values=decoder_outputs.past_key_values, # None
            decoder_hidden_states=decoder_outputs.hidden_states, # None
            decoder_attentions=decoder_outputs.attentions, # None
            encoder_last_hidden_state=encoder_outputs.last_hidden_state, # 1280, 20, 768
